...
# ✅ Qwen 配置 - 使用阿里云DashScope SDK
# 设置API Key:cite[4]
DASHSCOPE_API_KEY = " "  # 替换为你的阿里云API密钥
...
RESULT_CSV = "qwen_test_predictions.csv"  # 使用CSV格式保存结果
LOG_FILE = "processing_log.txt"
PROGRESS_FILE = "progress_checkpoint.txt"
MODEL_NAME = "qwen3"  # 使用Qwen模型:cite[4]   qwen-plus
...
def create_few_shot_messages(train_df, num_samples_per_label=2):
    # 获取数据集中的所有标签
...
# 使用上面定义的函数构造消息
few_shot_messages = create_few_shot_messages(train_df, num_samples_per_label=2)


# 增强的分类函数（带重试机制）:cite[4]
def classify_with_retry(text, few_shot_messages):
    messages = few_shot_messages + [{"role": "user", "content": text}]

    for attempt in range(MAX_RETRIES):
        try:
            # 使用DashScope SDK调用方式:cite[4]
            response = Generation.call(
                model=MODEL_NAME,
                messages=messages,
                temperature=0.0,
                max_tokens=10,
                result_format='message'  # 指定返回格式为消息格式:cite[4]
            )
            ...
            if response.status_code == HTTPStatus.OK:
                ...
                print(f"\n⚠️ API returned status code: {response.status_code}, error: {response.message}")
                # 根据错误类型决定是否重试
                if response.status_code >= 500:  # 服务器错误，可以重试
